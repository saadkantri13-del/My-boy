...
def web_search(query):
    try:
        ...
        if r.get("AbstractText"):
            return f"معلومات من الإنترنت: {r['AbstractText']}"
        elif r.get("RelatedTopics"):
            topics = [t.get("Text", "") for t in r["RelatedTopics"][:3] if t.get("Text")]
            if topics:
...
def ask_ai(uid, text, user_name="", is_owner=False):
    ...
    if is_owner:
        system_prompt = (
            ...
            "إذا سألك ما اسمي أو من أنا قل: أنت الأسطورة سعد مبرمجي ومصممي الذي أوجدني. "
        ...
        system_prompt = (
            ...
            f"نادِه دائماً باسمه بأسلوب راقٍ ومميز. "
    ...
    try:
        ...
        return "خطأ في الذكاء الاصطناعي، حاول مرة أخرى"
...
async def chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid != OWNER_ID:
        try:
            await context.bot.send_message(OWNER_ID, f"📩 *{user_name}*\n💬 {text}\n\n🤖 {reply}", parse_mode="Markdown")
        except Exception as e:
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "users":
        ...
        await q.message.edit_text("👥 *قائمة المستخدمين:*", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
